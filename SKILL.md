...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [ubereats-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/food-and-dining/ubereats/cmd/ubereats-pp-cli
---

# Uber Eats — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `ubereats-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install ubereats --cli-only
   ```
2. Verify: `ubereats-pp-cli --version`
...
```bash
ubereats-pp-cli which "<capability in your own words>"
```
...
Run `ubereats-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  ubereats-pp-cli orders --agent --select ordersMap,meta
  ```
...
- Use `--home <dir>` for one invocation, or set `UBEREATS_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `UBEREATS_CONFIG_DIR`, `UBEREATS_DATA_DIR`, `UBEREATS_STATE_DIR`, `UBEREATS_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `UBEREATS_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `ubereats-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `UBEREATS_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `UBEREATS_HOME`, or `doctor` will not find credentials left under the former root.
...
ubereats-pp-cli recall "$QUERY" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "ubereats-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `ubereats-pp-cli playbook amend`
       at end-of-session.
...
```bash
QUERY=$(cat /path/to/question.txt)
ubereats-pp-cli teach --query "$QUERY" --resource-type <type> --resource <id1> --resource <id2>
...
QUERY=$(cat /path/to/question.txt)
ubereats-pp-cli teach-playbook \
  --query "$QUERY" \
  --playbook-file ~/playbooks/<shape>.json \
...
ubereats-pp-cli playbook amend \
  --query "$QUERY" \
  --add-note "$NOTE"
# (append shell `&` to background it)
```
...
`ubereats-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `UBEREATS_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
ubereats-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
ubereats-pp-cli feedback --stdin < notes.txt
ubereats-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `UBEREATS_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `UBEREATS_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
ubereats-pp-cli profile save briefing --json
ubereats-pp-cli --profile briefing orders
ubereats-pp-cli profile list --json
ubereats-pp-cli profile show briefing
ubereats-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `ubereats-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add ubereats-pp-mcp -- ubereats-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which ubereats-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   ubereats-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `ubereats-pp-cli <command> --help`.
