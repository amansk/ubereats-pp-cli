...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Uber Eats",
...
func requiredAuthCookies() []string {
	raw := []string{"sid", "csid", "jwt-session"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for .ubereats.com.
...
profile by name when the installed backend supports it.`,
		Example: `  ubereats-pp-cli auth login --chrome
  ubereats-pp-cli auth login --browser
  ubereats-pp-cli auth login --cookies-file storage-state.json
  ubereats-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  ubereats-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  ubereats-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  ubereats-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			if v := os.Getenv("UBEREATS_COOKIES"); v != "" {
			...
			if !fromPressAuth && !fromCookiesFile {
				...
				if err != nil {
					...
					if runtime.GOOS == "windows" {
						...
						fmt.Fprintf(w, "\n  ubereats-pp-cli auth login --cookies-file <path>\n\n")
			...
			requiredCookies := []string{"sid", "csid", "jwt-session"}
			...
			cfg.CredentialDomain = ".ubereats.com"
...
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  ubereats-pp-cli auth refresh",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionTargetURL(cfg *config.Config) string {
	baseURL := strings.TrimRight("https://www.ubereats.com", "/")
	if cfg != nil && cfg.BaseURL != "" {
		baseURL = strings.TrimRight(cfg.BaseURL, "/")
	}
	validationPath := strings.TrimSpace("/_p/api/getUserV1")
	if validationPath == "" {
		return baseURL
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ubereats-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  ubereats-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .ubereats.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | ubereats-pp-cli auth set-token\n  ubereats-pp-cli auth set-token < token-file",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ubereats-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionProofStatusForAuth(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run ubereats-pp-cli auth login --chrome"
	}
	var proof browserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run ubereats-pp-cli auth login --chrome"
	}
	if proof.APIName != "ubereats" || proof.CookieDomain != ".ubereats.com" || proof.ValidationPath != "/_p/api/getUserV1" {
...
func validateAndWriteBrowserSessionProof(ctx context.Context, cfg *config.Config, flags *rootFlags) error {
	...
	method := strings.ToUpper(strings.TrimSpace("POST"))
	if method == "" {
		method = "GET"
	}
	if method != "GET" {
