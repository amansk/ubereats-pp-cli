...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UBEREATS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UBEREATS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ubereats-pp-cli/feedback")
...
When ` + "`UBEREATS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UBEREATS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  ubereats-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  ubereats-pp-cli feedback --stdin < notes.txt
  ubereats-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ubereats-pp-cli feedback list
  ubereats-pp-cli feedback list --limit 5
  ubereats-pp-cli feedback list --json`,
