...
type rootFlags struct {
	...
	// invocation. Mirrors the UBEREATS_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ubereats-pp-cli",
		Short: `Ubereats CLI — Your Uber Eats order history as a local database — spend, favourites, and search the app cannot answer.`,
		Long: `Ubereats CLI — Your Uber Eats order history as a local database — spend, favourites, and search the app cannot answer.
...
Health check: run 'ubereats-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ubereats-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select ordersMap,meta)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'ubereats-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelFindCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelHistoryCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSpendCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSyncCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTopItemsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTopRestaurantsCmd(flags))
