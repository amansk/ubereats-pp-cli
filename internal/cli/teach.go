...
// ubereats-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"ubereats-pp-cli/internal/learn/entities"
	"ubereats-pp-cli/internal/learn/lookups"
	"ubereats-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "UBEREATS_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("ubereats-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" ubereats-pp-cli teach --query "$QUERY" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" ubereats-pp-cli recall "$QUERY" --agent
  QUERY="$(cat /path/to/question.txt)" ubereats-pp-cli recall "$QUERY" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ubereats-pp-cli learnings list --agent
  ubereats-pp-cli learnings list --query "$QUERY"
  ubereats-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" ubereats-pp-cli learnings forget "$QUERY" --resource <id>
  QUERY="$(cat /path/to/question.txt)" ubereats-pp-cli learnings forget "$QUERY" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  ubereats-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  ubereats-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
