...
var whichIndex = []whichEntry{
	{Command: "sync", Description: "Walk every page of getPastOrdersV1 and mirror orders plus line items into SQLite, incrementally by default.", Group: "Local state that compounds", WhyItMatters: "Run it once and every other command works offline in milliseconds."},
	{Command: "history", Description: "List synced orders newest-first with date bounds, or fetch one order with its line items by id.", Group: "Local state that compounds", WhyItMatters: "Stable UUIDs and date filters make follow-up questions cheap."},
	{Command: "spend", Description: "Sum order totals into monthly or yearly buckets from the local mirror.", Group: "Analytics the app cannot do", WhyItMatters: "Answers 'how much did I spend on delivery this year' in one call."},
	{Command: "top-items", Description: "Rank line items by quantity across every synced order, with order counts and spend.", Group: "Analytics the app cannot do", WhyItMatters: "Grounds 'what do I usually get' in actual counts."},
	{Command: "top-restaurants", Description: "Rank restaurants by order count and total spend.", Group: "Analytics the app cannot do", WhyItMatters: "Shows where the money actually goes."},
	{Command: "find", Description: "Case-insensitive search across restaurant names and item titles in the local mirror.", Group: "Analytics the app cannot do", WhyItMatters: "'When did I last order X' becomes a single local query."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  ubereats-pp-cli which "stale tickets"
  ubereats-pp-cli which "bottleneck"
  ubereats-pp-cli which --limit 1 "send message"
  ubereats-pp-cli which                                # list the full capability index`,
