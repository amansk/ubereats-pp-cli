...
type Config struct {
	...
	ChromeProfile      string                      `toml:"chrome_profile,omitempty"`
	...
	UbereatsCookies  string          `toml:"cookies"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("UBEREATS_COOKIES"); v != "" {
		cfg.UbereatsCookies = v
		cfg.markEnvOverride("UbereatsCookies")
		cfg.AuthSource = "env:UBEREATS_COOKIES"
		cfg.CredentialSource = "env:UBEREATS_COOKIES"
	...
	if v := os.Getenv("UBEREATS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("UBEREATS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "ubereats-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.UbereatsCookies != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:UBEREATS_COOKIES"
		}
		return ensureAuthScheme("Bearer", c.UbereatsCookies)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.UbereatsCookies != "" {
		parts = append(parts, "cookies="+c.UbereatsCookies)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		UbereatsCookies:  c.UbereatsCookies,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.UbereatsCookies == "" {
		c.UbereatsCookies = creds.UbereatsCookies
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.UbereatsCookies = ""
	delete(c.envOverrides, "UbereatsCookies")
	c.updateFileConfigField("UbereatsCookies")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["UbereatsCookies"] {
			out.UbereatsCookies = c.fileConfig.UbereatsCookies
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "UbereatsCookies":
		c.fileConfig.UbereatsCookies = c.UbereatsCookies
	}
}
